...
void extract_inputbox_info(int img_height, int img_width, layer l, network* net, box *boxes, int nbox,  box *correct_boxes, float *conf_scores, int relative)
{
	int  i, j, n, best_n, ibox;
	int class_index, box_index, obj_index;
	float best_iou, iou, scale, prob;
	box truth, pred, best_pred;
	...
	for(ibox=0; ibox<nbox; ibox++)
	{
		...
		for(n = 0; n < l.n; ++n)
		{
			box_index = entry_index(l, 0, n*l.w*l.h + j*l.w + i, 0);
			pred = get_region_box(predictions, l.biases, n, box_index, i, j, l.w, l.h, l.w*l.h);
		...
		class_index = entry_index(l, 0, best_n*l.w*l.h + j*l.w + i, 5);
		box_index = entry_index(l, 0, best_n*l.w*l.h + j*l.w + i, 0);
		obj_index = entry_index(l, 0, best_n*l.w*l.h + j*l.w + i, 4);
        scale = l.background ? 1 : predictions[obj_index];
		prob = scale*predictions[class_index];
		conf_scores[ibox] = prob;
		correct_boxes[ibox] = best_pred;
	}
	correct_region_boxes(correct_boxes, nbox, img_width, img_height, net->w, net->h, 1);
}
...
void forward_region_layer(const layer l, network net)
{
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        ...
        for(t = 0; t < 30; ++t){
            ...
            if(!truth.x || !truth.y) break;
            if(truth.x>=1 || truth.y>=1 || truth.x<0 || truth.y<0) break;
            float best_iou = 0;
            int best_n = 0;
            i = (truth.x * l.w);
            j = (truth.y * l.h);
    ...
    *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
    if(count>0 && class_count>0) printf("Region Avg IOU: %f, Class: %f, Obj: %f, No Obj: %f, Avg Recall: %f,  count: %d\n", avg_iou/count, avg_cat/class_count, avg_obj/count, avg_anyobj/(l.w*l.h*l.n*l.batch), recall/count, count);
}
...
void correct_region_boxes_reverse(box *boxes, int n, int w, int h, int netw, int neth, int relative)
{
    int i;
    int new_w=0;
    int new_h=0;
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        b.x  = (b.x*new_w/w + (netw - new_w)/2.0) / netw;
        b.y  = (b.y*new_h/h + (neth - new_h)/2.0) / neth;
        b.w *= (float)new_w/(netw*w);
        b.h *= (float)new_h/(neth*h);
...
int get_region_boxes_face(layer l, int w, int h, int netw, int neth, float thresh, float **probs, box *boxes, box *mark_boxes, int only_objectness, int *map, float tree_thresh, int relative)
{
    int i,j,n,z;
    float *predictions = l.output;
    int candidate_num=0;
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            int index = n*l.w*l.h + i;
            for(j = 0; j < l.classes; ++j){
                probs[candidate_num][j] = 0;
            }
            int obj_index = entry_index(l, 0, n*l.w*l.h + i, 4);
            int box_index = entry_index(l, 0, n*l.w*l.h + i, 0);
            float scale = l.background ? 1 : predictions[obj_index];
            boxes[candidate_num] = get_region_box(predictions, l.biases, n, box_index, col, row, l.w, l.h, l.w*l.h);
            mark_boxes[candidate_num] = boxes[candidate_num];
			...
			for(j = 0; j < l.classes; ++j){
				int class_index = entry_index(l, 0, n*l.w*l.h + i, 5 + j);
				float prob = scale*predictions[class_index];
				probs[candidate_num][j] = (prob > thresh) ? prob : 0;
				if(prob > max) max = prob;
			}
			probs[candidate_num][l.classes] = max;
            if(max>thresh) candidate_num++;
        }
    }
    correct_region_boxes(boxes, candidate_num, w, h, netw, neth, relative);
